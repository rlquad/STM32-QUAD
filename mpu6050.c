...
void print_ang(int choice,int dt)
{
    ...
    if(choice==0)
        ...
        sprintf(dummy,"ROLL %f PITCH %f YAW %f \r\n",roll*degconvert,pitch*degconvert,yaw*degconvert);
...
void TIM6_DAC1_IRQHandler(void)
{
   ...
   // print_ang(2,TIM_GetCounter(TIM2)-tim2);
 ...
 uint16_t TimerPeriod = (16000000/250) - 1;
  ...
  {
  Channel1Pulse = (uint16_t) ((uint32_t) 26 * (TimerPeriod - 1 ) / 100) - 1;
  Channel2Pulse = (uint16_t) ((uint32_t) 26 * (TimerPeriod - 1 ) / 100) - 1;
  Channel3Pulse = (uint16_t) ((uint32_t) 26 * (TimerPeriod - 1 ) / 100) - 1;
  Channel4Pulse = (uint16_t) ((uint32_t) 26 * (TimerPeriod - 1 ) / 100) - 1;
  }
